...
module.exports = app => {
  app.get(["/logs", "/logs/*"], (req, res)=>{
    ...
    appbk = appbk ? `background: ${appbk};` : 'background-image: linear-gradient(110deg, var(--secd-fc) 25%, transparent 0%),linear-gradient(315deg, transparent 30%, var(--tras-bk) 30%),linear-gradient(45deg, transparent 66%, var(--secd-bk) 0%),linear-gradient(333deg, var(--icon-bk) 53%, transparent 0%);background-attachment: fixed;'
    style = `:root {${ d_color.join('') };--main-fc: #FAFAFD;--delt-bk: #FF2828;--icon-run: #66FF00;--icon-bk: #66FF0088;--tras-bk: #A7A8BD88;scroll-behavior: smooth;}body{margin: 0;padding: 0;height: 100vh;}#app{padding: 3px 8px;box-sizing: border-box;min-height: 100vh;${appbk}}${style}`
