...
// 3.1.8 版本添加了默认的 PM2 执行方式，建议在此版本后使用。
  ...
  for (let file of tree) {
    if (file.type === 'blob') {
      if (noupdate.filter(item=>file.path.match(item)).length === 0) {
        ...
        await $download(durl, file.path).then(d=>console.log('文件已下载: ' + d)).catch(e=>console.error(e))
      }
    }
  }
...
}).catch(e=>console.error('elecV2P 软更新文件失败', e.message || e))
