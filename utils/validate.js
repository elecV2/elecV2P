...
function isAuthReq(req, res) {
  ...
  if (cookies.token && (CONFIG.wbrtoken + CONFIG.wbrtoken).indexOf(atob(cookies.token)) !== -1) {
  ...
  if (CONFIG.wbrtoken) {
    ...
    if (token === CONFIG.wbrtoken) {
      ...
      if (res) {
        ...
        res.setHeader('Set-Cookie', cookie.serialize('token', btoa((CONFIG.wbrtoken + CONFIG.wbrtoken ).substr(iRandom(CONFIG.wbrtoken.length), 8)), {
          // httpOnly: true,
